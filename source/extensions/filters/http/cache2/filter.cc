                               ...
                               bool  is_last ) -> Http::FilterHeadersStatus
{
  ENVOY_LOG (
    trace,
    "$$$ decoding headers = [host=\"{}\", path=\"{}\"  // state = {}, stream id = {}",
  ...
  if (
    ! response
    || std::chrono::system_clock::now () - (*response) -> m_Stamp > 10s  // pretend entries older than 10s are expired
  )
  {
    m_State = State::Miss;
    return  Http::FilterHeadersStatus::Continue;
                               ...
                               bool  is_last ) -> Http::FilterHeadersStatus
{
  ENVOY_LOG (
    trace,
    "$$$ encoding headers = [status={}, ...]  // state = {}, stream id = {}",
...
[[nodiscard]]
auto  Filter::derive_key  ( const Http::RequestHeaderMap & headers ) -> Self::Key
{
  return  { std::string { headers . getHostValue () }, std::string { headers . getPathValue () } };
}
